...
public class KestrelClient {
	...
	private class MixedInputStream { // For dual binary and ASCII text streams.
		...
		public byte[] readFixedLength(int num_bytes) throws IOException,
				ParseError {
		...
		public String readLine() throws ParseError, IOException {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();

			while (true) {
				int val = _is.read();
				if (val == -1) {
					throw new ParseError("EOF while expecting line. Read " + baos.toByteArray().length + " bytes so far: " + new String(baos.toByteArray()));
	...
	public KestrelClient(String hostname, int port) throws IOException {
		_sock = new Socket();
	...
	private Item parseDequeueOutputItem(String[] header_tokens)
		...
		if (header_tokens.length != 4) {
			throw new ParseError();
		}

		int id = 0;
		String id_line = _mis.readLine();
		byte[] data = id_line.getBytes();
	...
	private boolean parseBooleanOutput(String expected) throws IOException,
			ParseError {
	...
	private Item parseDequeueOutput() throws IOException, ParseError {
		String header = _mis.readLine();
	...
	public Item dequeue(String queueName) throws IOException, ParseError {
		_os.write(dequeueCommand(queueName));
		return parseDequeueOutput();
	}

	public boolean fail(String queueName, int id) throws ParseError,
			IOException {
		_os.write(failCommand(queueName, id));
		return parseBooleanOutput("TRANSACTION_FAIL");
	}

	public boolean ack(String queueName, int id) throws ParseError, IOException {
		_os.write(ackCommand(queueName, id));
		return parseBooleanOutput("TRANSACTION_ACK");
	}

	public boolean queue(String queueName, String val) throws ParseError,
			IOException {
